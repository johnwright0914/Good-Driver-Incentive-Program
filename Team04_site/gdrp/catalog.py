import json
import os
from .templatetags.views.py import getConnection
from audit import log_item_added, log_item_removed
...
class CatalogController():
    ...
    def insert(self, item: dict, sponsor_id):
        ...
        try:
            ...
            log_item_added(item['name'], item['price'])
            return True
    ...
    def unlist_product(self, product_id):
        ...
        try:
            ...
            print(e)
            return False
    #Needs to be updated, no longer using etsy
...
def clear_catalog(self, sponsor_id):
    """ Remove all items from the catalog for the given sponsor """
    sql = "DELETE FROM product WHERE sponsor_id=%s"
    vals = (sponsor_id, )
       
def change_category(self, product_id, new_category):
        """Change the category of a product in the database"""
        sql = "UPDATE product SET category = %s WHERE product_id = %s"
        vals = (new_category, product_id)
...
def mark_as_archived(self):
        if self.status in ['deployed', 'undeployed']:
